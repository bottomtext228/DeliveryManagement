...
// we use forwardRef to pass the ref from the parent
        ...
        <div className="w-full mt-4">
            {fileSrc &&
                <div className="w-full h-full">
                    <img className="object-contain w-full h-full border border-gray-200 rounded-xl p-2 mb-4" src={fileSrc}></img>
                </div>
            }
            {error && <div className="text-red-500">{error.message}</div>}
            <label htmlFor={id} className="flex w-full h-14.5 border border-gray-300 rounded-lg cursor-pointer hover:shadow-sm active:shadow-md">
                <div className="flex items-center justify-start p-3 overflow-hidden flex-4/5 text-ellipsis whitespace-nowrap">
            ...
            <input
                ...
                className="w-0 h-0 opacity-0 overflow-hidden absolute -z-[1]" onChange={
                    (e: React.ChangeEvent<HTMLInputElement>) => {
                        onChange?.(e); // call default RHF handler
